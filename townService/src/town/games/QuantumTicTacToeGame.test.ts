...
describe('QuantumTicTacToeGame', () => {
  ...
  describe('_leave', () => {
    describe('when two players are in the game', () => {
      ...
      it('should set the game to WAITING_TO_START and reset the state when both players leave', () => {
        game.leave(player2);
        game.leave(player1);
        expect(game.state.status).toBe('WAITING_TO_START');
        expect(game.state.winner).toBe(undefined);
        expect(game.state.x).toBe(undefined);
        expect(game.state.o).toBe(undefined);
      });
    });
  ...
  describe('applyMove', () => {
    ...
    describe('scoring and game end', () => {
      ...
      it('should not allow moves on a board that has been won', () => {
        ...
        // @ts-expect-error - private property
        expect(game._games.A._board[1][2]).toBe('');
